...
- [n] gt -- 切换到下一个标签。如果前面加了  ， 就切换到第n个标签。第一个标签的序号就是1。
    
    n
    
- :tab split --5 将当前缓冲区的内容在新页签中打开。
- :tabc[lose] -- 关闭当前的标签页。
- :tabo[nly] -- 关闭其它的标签页。
- :tabs -- 列出所有的标签页和它们包含的窗口。
- :tabm[ove] [N] -- 移动标签页，移动到第N个标签页之后。 如  当前标签页，就会变成第一个标签页。
...
[Vim使用笔记 - JollyWing - 博客园](https://www.cnblogs.com/jiqingwu/archive/2012/06/14/vim_notes.html#id59)